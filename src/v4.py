...
class ManhwaTranslatorLongImage:
    ...
    def extract_text_from_chunk(self, chunk_image):
        ...
        try:
            ...
            for idx, processed_img in enumerate(processed_images):
                ...
                # OCR avec paramètres ENCORE PLUS AGRESSIFS
                result = self.reader.readtext(
                    temp_path,
                    detail=1,
                    paragraph=False,
                    min_size=3,  # Texte encore plus petit accepté
                    text_threshold=0.4,  # Encore plus permissif
                    low_text=0.2,  # Détecte beaucoup plus de texte
                    link_threshold=0.2,  # Lie plus facilement les caractères
                    canvas_size=3200,  # Canvas encore plus grand
                    mag_ratio=1.8  # Agrandissement plus fort
                )
    ...
    def create_text_mask(self, image_shape, bbox, expansion=12):
        """
        Crée un masque pour la zone de texte PLUS LARGE
        
        Args:
            image_shape: Forme de l'image (height, width)
            bbox: Boîte englobante du texte
            expansion: Pixels à ajouter autour du texte (augmenté à 12)
        ...
        # Calculer le centre et agrandir BEAUCOUP la région
        ...
        for point in points:
            ...
            if dist > 0:
                # Expansion plus agressive pour couvrir les contours
                expanded_point = center + direction * (1 + (expansion * 2.5) / dist)
            else:
        ...
        # Dilater ENCORE PLUS le masque pour être sûr de tout effacer
    ...
    def detect_bubble_color(self, image_cv, bbox):
        """
        Détecte la couleur de fond de la bulle AMÉLIORÉ
        Échantillonne une zone PLUS LARGE pour mieux détecter la couleur
        ...
        # Agrandir la région d'échantillonnage
        width = x_max - x_min
        height = y_max - y_min
        padding = int(max(width, height) * 0.3)  # 30% de padding
        
        x_min_padded = max(0, x_min - padding)
        x_max_padded = min(image_cv.shape[1], x_max + padding)
        y_min_padded = max(0, y_min - padding)
        y_max_padded = min(image_cv.shape[0], y_max + padding)
        
        region = image_cv[y_min_padded:y_max_padded, x_min_padded:x_max_padded]
        ...
        # Méthode 1: Prendre les pixels très clairs (blanc/crème de bulle)
        very_bright_mask = gray_region > 220
        if very_bright_mask.sum() > region.size * 0.1:  # Au moins 10% de pixels clairs
            bright_pixels = region[very_bright_mask]
            avg_color = bright_pixels.mean(axis=0)
            return tuple(map(int, avg_color))
        
        # Méthode 2: Prendre les pixels clairs (> 200)
        ...
        # Méthode 3: Si pas de pixels clairs, analyser l'histogramme
        hist = cv2.calcHist([gray_region], [0], None, [256], [0, 256])
        # Trouver le pic dans les valeurs claires (180-255)
        bright_hist = hist[180:256]
        if bright_hist.sum() > 0:
            peak_value = 180 + np.argmax(bright_hist)
            peak_mask = (gray_region >= peak_value - 10) & (gray_region <= peak_value + 10)
            if peak_mask.sum() > 0:
                peak_pixels = region[peak_mask]
                avg_color = peak_pixels.mean(axis=0)
                return tuple(map(int, avg_color))
    ...
    def simple_erase_text(self, image_cv, mask, bbox):
        """
        Efface le texte en REMPLISSANT avec la couleur de la bulle
        VERSION AMÉLIORÉE avec traitement des bords
        ...
        try:
            ...
            # Remplir avec cette couleur
            image_cv[mask > 0] = bubble_color
            
            # Appliquer un flou gaussien PLUS FORT sur les bords pour mélanger
            kernel = np.ones((5, 5), np.uint8)
            mask_edges = cv2.dilate(mask, kernel, iterations=2) - cv2.erode(mask, kernel, iterations=2)
            
            if mask_edges.sum() > 0:
                # Flou plus fort pour bien mélanger les bords
                blurred = cv2.GaussianBlur(image_cv, (7, 7), 0)
                image_cv = np.where(mask_edges[:, :, None] > 0, blurred, image_cv)
                
                # Deuxième passe de flou plus léger sur une zone encore plus large
                mask_edges2 = cv2.dilate(mask_edges, kernel, iterations=1)
                blurred2 = cv2.GaussianBlur(image_cv, (5, 5), 0)
                image_cv = np.where(mask_edges2[:, :, None] > 0, blurred2, image_cv)
